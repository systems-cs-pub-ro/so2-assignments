...
#
# Script to automatically create git repo for SO2 assignments
# based on so-create-repo.sh
#
...
{
        # checks if there already is an "l4-so2-assignments" repo
        ...
        # search "l4-so2-assignments" repo
...
create_so2_assignment_clone()
{
        ...
        cd $REPO_NAME
        git remote add upstream $SO2_ASSIGNMENTS_URL
        cd -

        echo -e "Your $REPO_NAME local clone was successfully created!\n"
}

update_so2_assignment_clone()
{
...
add_asist_so2_assignment_group()
{
        echo "Adding the SO2 team members as reporters ..."

        # based on REPO_ID we add new project members as reporters
        for member in ${SO2_TEAM[@]}; do
                res=$(curl -sS -H "PRIVATE-TOKEN: $PRIVATE_TOKEN"           \
...
# check if the user has the "l4-so2-assignments" repo on GitLab
check_existing_assignment_repo
...
if [ $? -ne 0 ]; then
        ...
        create_so2_assignment_repo $use_ssh
